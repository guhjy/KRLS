plotcin <- function (x, which = c(1:2), main = "distributions of pointwise marginal effects", 
    ...
    probs = c(0.25, 0.75), ...) 
{
    ...
    if (is.numeric(setx)) {
        ...
        if (length(setx) != 1) {
            stop("setx must be one of mean or median")
        }
        if (sum(setx %in% c("mean", "median")) < 1) {
            stop("setx must be one of mean or median")
        }
    ...
    if (is.null(x$derivatives)) {
        ...
        if (1 %in% which) {
            form <- as.formula(paste("~", paste(colnames(x$derivatives), 
                collapse = "+"), sep = ""))
            requireNamespace("lattice", quietly = TRUE)
            print(lattice::histogram(form, data = data.frame(x$derivatives), 
                breaks = NULL, main = main, ...))
        }
    }
    if (2 %in% which) {
        ...
        binaryindicator <- which(apply(x$X, 2, lengthunique) == 
            2)
        quantiles <- apply(x$X, 2, quantile, probs = probs)
        for (i in 1:d) {
            if (i %in% binaryindicator) {
                ...
                Newdata <- matrix(rep(setx, nvalues), ncol = d, 
                  byrow = T)
                Newdata[, i] <- Xi
            }
